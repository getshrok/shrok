...
export const api = {
  ...
  schedules: {
    ...
    create: (body: { headId: string; taskName?: string; kind?: 'task' | 'reminder' | 'script'; cron?: string; runAt?: string; conditions?: string; agentContext?: string; relayGuidance?: string; requiresAck?: boolean; nagIntervalMinutes?: number; startAt?: string; deliverToHeadIds?: string[]; endDate?: string }) =>
      request<{ schedule: Schedule }>('/api/schedules', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }),
    update: (id: string, patch: { enabled?: boolean; cron?: string; runAt?: string; conditions?: string; agentContext?: string; relayGuidance?: string; requiresAck?: boolean; nagIntervalMinutes?: number | null; deliverToHeadIds?: string[]; endDate?: string | null }) =>
