...
export const api = {
  ...
  schedules: {
    ...
    create: (body: { headId: string; taskName?: string; kind?: 'task' | 'reminder'; cron?: string; runAt?: string; conditions?: string; agentContext?: string; requiresAck?: boolean; nagIntervalMinutes?: number; startAt?: string }) =>
      request<{ schedule: Schedule }>('/api/schedules', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }),
    update: (id: string, patch: { enabled?: boolean; cron?: string; runAt?: string; conditions?: string; agentContext?: string; requiresAck?: boolean; nagIntervalMinutes?: number | null }) =>
