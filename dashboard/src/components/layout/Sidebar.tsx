...
export default function Sidebar({ onSettingsOpen }: { onSettingsOpen: () => void }) {
  ...
  return (
    <aside className={`${collapsed ? 'w-14' : 'w-56'} bg-zinc-900 border-r border-zinc-800 flex flex-col transition-[width] duration-150`}>
      <div className={`${collapsed ? 'px-2 justify-center' : 'px-4 justify-between'} py-5 border-b border-zinc-800 flex items-center`}>
        {collapsed ? (
      ...
      <nav className="flex-1 px-2 py-4 space-y-1">
        {[
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center ${collapsed ? 'justify-center' : 'gap-2.5'} px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                isActive
      ...
      <div className="px-2 py-4 border-t border-zinc-800 space-y-1">
        ...
        <button
          ...
          className={`w-full flex items-center ${collapsed ? 'justify-center' : 'gap-2.5'} px-3 py-2 my-[2px] rounded-md text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 transition-colors`}
        >
        ...
        <button
          ...
          className={`w-full flex items-center ${collapsed ? 'justify-center' : 'gap-2.5'} px-3 py-2 my-[2px] rounded-md text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 transition-colors`}
        >
          {collapsed ? <PanelLeftOpen size={16} /> : <><PanelLeftClose size={16} className="shrink-0" /> Collapse</>}
        </button>
      </div>
    </aside>
  )
