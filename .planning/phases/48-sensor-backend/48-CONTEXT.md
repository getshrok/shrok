...
- **D-08** (SENSOR-11): The **same** ambient scan feeds **all** consumers — factor it into one shared function. There are **THREE** read sites today (research finding — the discussion only named two): (1) the head assembler (`src/head/assembler.ts`, the head's own turns); (2) the proactive scheduler (`src/head/activation.ts` `readAmbientContext()` → `src/scheduler/proactive.ts` `ambientContext`); and (3) **sub-agent system prompts** (`src/sub-agents/tool-surface.ts:78`). All three are repointed at the shared scan to preserve current behavior parity (sub-agents see ambient state today and should continue to) — and at each site the block must sit in the **uncached** region, never busting that surface's cache prefix.
...
- `src/sub-agents/tool-surface.ts` (~line 78) — the **third** `AMBIENT.md` read site, feeding spawned sub-agent system prompts. Repoint to the shared scan, uncached (D-08); delete the single-file read (D-09).
- `src/sub-agents/registry.ts` — `executeBash` is the **exact child-process pattern to mirror** for the runner (`execFile` with a `timeout` option, both branches `resolve()` so it never throws out of the scheduler tick — research finding).
