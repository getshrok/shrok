...
**Progress:** Phase 40 (Config & Adapter Skeleton) ✓ · Phase 41 (Inbound Synchronous Reply Endpoint) ✓ — `/v1/chat/completions` is live on the dashboard Express server (port 8888): bearer auth (self-contained JSON-401 boundary), CSRF `/v1/*` exclusion, `pendingReply` held-connection slot with a ~3s deadline (D-01: real reply or lapse, no filler), and the captured-not-applied Apache bypass snippet for Phase 43. Next: Phase 42 (Outbound HA REST Announce), Phase 43 (E2E smoke test + setup docs).
...
- v1.5 Home Assistant Voice **in progress** — HACF-01/02 validated in Phase 40 (config + adapter skeleton, see Validated). Remaining: **HACV-01–06** inbound synchronous `/v1/chat/completions` reply endpoint (Phase 41), **HAAN-01–03** outbound HA `assist_satellite` announce/start_conversation (Phase 42), **HADOC-01** end-to-end smoke test + setup docs (Phase 43).
