...
| Nag fields are creation-only (`SchedulePatch`/`update()` exclude them) (D-09) | No edit path keeps the preservation invariant simple; toggling ack post-create is out of scope | Validated in Phase 37 |
...
---
*Last updated: 2026-05-23 — Phase 37 complete (Schema & Tool Params): `requiresAck` + `nagIntervalMinutes` added to the reminder Schedule schema with mtime-stable lazy JSON migration (`migrateLegacySchedule`), `create_reminder` wired with the four ack/nag params + boundary validation, and the tool description reworded for `triggerAt`+`cron` start-then-repeat (SCHED-04). 8/8 must-haves verified; the forward-looking nag description was ratified per D-10b (REVIEW CR-01 noted, not actioned). Runtime nag re-arm + ack tool/agent are Phase 38. Milestone v1.4 Unmissable Reminders in progress (sourced from SEED-001; bundled with backlog 999.1 dashboard start-date inputs); v1.3 was never formally closed via /gsd-complete-milestone (status was `verifying`, audit + live smoke tests pending). Prior: Phase 36 complete — inbound sender attribution (`InboundMessage.senderName?: string` contract, `normalizeSenderName` + `buildPrefixedText` in `src/head/sender-prefix.ts`, central prefix construction in `headRouteMessage`, D-11 stripper `stripLeadingBracketPrefixes`, five adapters populate senderName, voice/dashboard/webhook untouched per D-06). Phase 35 complete: per-head scheduling.*
