...
---
*Last updated: 2026-05-14 — Phase 35 complete: per-head scheduling — Schedule.headId at storage type, lazy JSON migration, ScheduleEvaluator per-event enqueue, agent tool factory headId requirement, update_schedule reassignment reject, reminder fire first-channel fallback, dashboard schedules API head-list validation + DELETE cascade with counts, UI head picker + Head column, architectural regression test pins cross-head schedule isolation. Phase 34 WR-03 NOTE closed. Milestone v1.3 ready for completion (audit + smoke tests pending).*
