...
describe('cancel propagation — retract during in-flight bash', () => {
  it('retract while bash is sleeping terminates the agent within 500ms', async () => {
    ...
    const agentId = await runner.spawn({ task: 'run sleep', name: 'sleeper', trigger: 'manual', headId: 'default' })
...
describe('cancel propagation — mid-round retract', () => {
  it('retract between LLM rounds bounds the LLM call count', async () => {
    ...
    const { runner, agentStore } = makeRunner(llmRouter)
    const agentId = await runner.spawn({ task: 'tight loop', name: 'looper', trigger: 'manual', headId: 'default' })
...
describe('cancel propagation — cooperative retract before tool call', () => {
  it('retract before any llm.complete still reaches retracted via inbox poll', async () => {
    ...
    const { runner, agentStore } = makeRunner(llmRouter)
    const agentId = await runner.spawn({ task: 'quick', name: 'quick', trigger: 'manual', headId: 'default' })
...
describe('cancel propagation — ctx.abortSignal reaches child_process.execFile', () => {
  it('aborted bash surfaces the "terminated by agent cancel" marker', async () => {
    ...
    const { runner, agentStore } = makeRunner(llmRouter)
    const agentId = await runner.spawn({ task: 'run sleep', name: 'sleeper-2', trigger: 'manual', headId: 'default' })
...
describe('mid-loop retract via onRoundComplete callback', () => {
  it('retract written to inbox during a long tool-call sequence yields status=retracted (not failed)', async () => {
    ...
    const agentId = await runner.spawn({ task: 'tight loop for mid-loop retract', name: 'midloop-retract', trigger: 'manual', headId: 'default' })
