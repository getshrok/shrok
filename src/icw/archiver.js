...
async function archiveTopic(topicId, llm, store, config, logger, prompts) {
    ...
    for (const c of allChunks) {
        if (c.coversChunkIds)
            c.coversChunkIds.forEach(id => alreadyCovered.add(id));
    }
    ...
    if (heavy.length > 0) {
        ...
        try {
            ...
            logger.warn(`[archiver] heavy archival failed for "${topicId}" — keeping originals (${err.message})`);
        }
        if (summary) {
    ...
    if (moderate.length > 0) {
        ...
        try {
            ...
            logger.warn(`[archiver] moderate archival failed for "${topicId}" — keeping originals (${err.message})`);
        }
        if (summary) {
            await store.appendChunk(topicId, {
    ...
    for (const c of updatedChunks) {
        if (c.coversChunkIds)
            c.coversChunkIds.forEach(id => newCovered.add(id));
    }
    const effectiveChunks = updatedChunks.filter(c => !c.coversChunkIds || c.coversChunkIds.length > 0)
        .filter(c => !newCovered.has(c.chunkId));
    const tokenEst = config.tokenCounter(effectiveChunks.flatMap(c => c.messages).map(m => m.content).join('\n'));
...
export async function archive(llm, store, config, logger, topicId, prompts) {
    if (topicId) {
