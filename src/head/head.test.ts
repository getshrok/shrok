...
describe('HeadToolExecutor', () => {
  ...
  it('message_agent forwards the head onVerbose so a continued agent re-binds its xray channel', async () => {
    ...
    await verboseExecutor.execute({ id: 'tc1', name: 'message_agent', input: { agentId: 't1', message: 'go ahead, the window seat under $300' } })
    expect(runner.update).toHaveBeenCalledWith('t1', expect.stringContaining('go ahead, the window seat under $300'), onVerbose)
  })
