...
export function buildHeadSpawnAgentDef(agentModelDynamic: boolean): ToolDefinition {
  const baseDesc = 'Spawn an agent to handle a task asynchronously. Your job is to RELAY, not to author: pass the user\'s request through in their own words via `task`, and paste the relevant conversation verbatim into `context` — let the natural conversation be the agent\'s prompt rather than writing a fresh one. Tell the agent what is wanted, not how to do it; the agent decides the approach.'
  ...
  const properties: Record<string, unknown> = {
    ...
    task: { type: 'string', description: 'What the agent must accomplish, stated as the ask itself. Lead with the user\'s own words — quote them. Your only job here is to resolve what the agent can\'t see (pronouns, "that thing", which of several options) into concrete terms; write original prose only when the user\'s words alone wouldn\'t make the goal clear. Do not invent an approach, add steps, or prescribe how — the agent decides that. The test: every phrase in `task` should be traceable to something the user actually said — if it isn\'t, you\'re authoring, not relaying. Example — user said: "Look up the NVIDIA stock price currently." Bad: "Look up the current NVIDIA stock price. Search the web for the latest price and return the price plus any recent movement (% change today)." (invents a method and a return-scope the user never asked for). Good: "Look up the current NVIDIA stock price." (mirrors the ask — nothing added).' },
