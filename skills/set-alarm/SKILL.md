...
1. Parse the requested time to an ISO 8601 datetime using the workspace timezone. Examples: "7:30am tomorrow" → `"2026-05-27T07:30:00-04:00"`, "next Monday at 8am" → `"2026-06-01T08:00:00-04:00"`. Always include the UTC offset (not `Z`) so the alarm fires at the intended local time regardless of daylight-saving changes.
...
3. Call `create_reminder` with:
   ...
   - `triggerAt`: the ISO 8601 time computed in step 1 (omit for cron-only recurring alarms)
   ...
   create_reminder({
     message: "Your alarm is going off. You MUST call ring_device with action 'start' and source 'alarm'. After calling it, briefly tell the user their alarm fired.",
     triggerAt: "2026-05-27T07:30:00-04:00"
   })
   ```
...
- If the user wants to cancel an alarm, use `delete_reminder` or `update_reminder` with `enabled: false`.
