...
- **After every push, watch CI until it goes green — a push isn't done until CI passes.** Run `gh run watch <run-id> --exit-status` (or poll `gh run list --branch main --limit 1`) on the run your push triggered; don't walk away at `git push`. If any job goes red, treat it as your problem regardless of whether *your* change caused it: investigate (`gh run view --job <id> --log`), reproduce locally (`npx vitest run …`, `npx tsc --noEmit`), fix, and push again until green. This matters doubly here because **a red run means the `build` job never runs, so `dashboard/dist/` is never rebuilt** — the dashboard changes you just pushed will not reach `main` or any deployed instance until CI is green. Don't deploy/pull a commit whose CI is still red or pending.
